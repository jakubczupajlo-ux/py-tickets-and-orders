...
class CinemaHall(models.Model):
    ...
    @property
    def capacity(self): return self.rows * self.seats_in_row
    def __str__(self): return self.name
...
class MovieSession(models.Model):
    ...
    def __str__(self): return f"{self.movie.title} {self.show_time.strftime('%Y-%m-%d %H:%M:%S')}"
...
class Order(models.Model):
    ...
    class Meta:
        ordering = ["-created_at"]
    def __str__(self): return f"{self.created_at.strftime('%Y-%m-%d %H:%M:%S')}"
...
class Ticket(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["row", "seat", "movie_session"], name="unique_ticket_session_seat")
        ]
